...
public class LanguageConf {
	...
	private List<LanguageItem> langList = new ArrayList();
	...
	public String getLangText(String text) {
		...
		if (this.langMap == null) {
			Map<String, LanguageItem> langMapTmp = new HashMap();
		...
		if (ok == null)
			ok = "";
		return String.format(ok, params.toArray());
	}
	...
	String fixStr(String str, List<String> params) {
		if (str.length() > 0) {
			...
			Ykh ykh = new Ykh(params);
			Ykh fkh = new Ykh(params);
