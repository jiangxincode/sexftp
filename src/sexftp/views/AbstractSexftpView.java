...
public class AbstractSexftpView extends ViewPart implements Consoleable, SrcViewable {
	...
	private void makeActions() {
		...
		this.actionRefreshFile
				.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin("sexftp", "/icons/refresh (5).gif"));
	...
	public void expandToObject(String filePath) {
		...
		int j = (arrayOfObject = this.viewer.getExpandedElements()).length;
		for (int i = 0; i < j; i++) {
			Object o = arrayOfObject[i];

			if ((o instanceof TreeParent)) {
				TreeObject[] arrayOfTreeObject;
				int m = (arrayOfTreeObject = ((TreeParent) o).getChildren()).length;
				for (int k = 0; k < m; k++) {
					TreeObject to = arrayOfTreeObject[k];
	...
	private void expandToObject(TreeObject to, String filePath) {
		if ((to instanceof TreeParent)) {
			if ((to.getO() instanceof FtpUploadConf)) {
				if (filePath.startsWith(((FtpUploadConf) to.getO()).getClientPath())) {
					this.viewer.expandToLevel(to.getParent(), 1);
					return;
				}
			}
			TreeObject[] arrayOfTreeObject;
			int j = (arrayOfTreeObject = ((TreeParent) to).getChildren()).length;
			for (int i = 0; i < j; i++) {
				TreeObject co = arrayOfTreeObject[i];
	...
	public boolean showQuestion(final String message) {
		Runnable runnable = new Runnable() {
			...
			public void run() {
				this.q = MessageDialog.openQuestion(AbstractSexftpView.this.viewer.getControl().getShell(),
	...
	public MessageDialogWithToggle showQuestionInDTread(final String message, final String toggleMessage) {
		Runnable runnable = new Runnable() {
			private MessageDialogWithToggle s = null;

			public void run() {
				this.s = AbstractSexftpView.this.showQuestion(message, toggleMessage);
			}
		...
		if (this.console == null) {
			...
			try {
				this.console.console("Welcome to Sexftp - "
						+ Platform.asLocalURL(Platform.getBundle("sexftp").getEntry("")).getFile());
			} catch (IOException e) {
